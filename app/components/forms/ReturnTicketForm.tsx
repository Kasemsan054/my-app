...
export default function ReturnTicketForm({ companies, products, currentUser, staffs = [] }: ReturnTicketFormProps) {
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full gap-6">
      ...
      {/* Left Column: Form */}
      <div className="w-full lg:w-[50%] bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2 text-slate-900 font-extrabold text-base">
                <FileBox className="text-brand-primary" size={20} /> 1. ข้อมูลใบแจ้งเปิดงานซ่อม
              </div>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2 text-slate-900 font-extrabold text-base">
                <Users className="text-brand-primary" size={20} /> 2. เจ้าหน้าที่ผู้รับเรื่อง
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <InputField label="เจ้าหน้าที่รับเครื่อง (หลัก)" disabled value={currentUser?.name || ''} />
              <SelectField 
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2 text-slate-900 font-extrabold text-base">
                <Wrench className="text-brand-primary" size={20} /> 3. ข้อมูลลูกค้าและอุปกรณ์
              </div>
            </div>
      ...
      <div className="w-full lg:w-[50%] bg-white p-6 md:p-8 rounded-3xl border border-slate-200/80 shadow-xs flex flex-col min-h-[500px]">
        ...
        {previewUrl ? (
          <div className="flex-1 flex flex-col space-y-4">
            ...
            <iframe src={previewUrl} className="w-full flex-1 min-h-[550px] rounded-2xl border border-slate-200 shadow-inner" />
          </div>
        ) : (
          <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-2xl p-8 text-center bg-slate-50/50">
            <div className="w-12 h-12 rounded-2xl bg-brand-primary-light text-brand-primary flex items-center justify-center mb-3">
              <Eye size={24} />
            </div>
            <div className="text-sm font-bold text-slate-700">ยังไม่มีการพรีวิวเอกสาร</div>
            <div className="text-xs text-slate-500 max-w-xs mt-1">
              กรอกข้อมูลในฟอร์มด้านซ้าย แล้วกดปุ่ม &quot;ดูพรีวิว PDF&quot; เพื่อตรวจสอบความถูกต้องของเอกสาร
            </div>
          </div>
        )}
      </div>
